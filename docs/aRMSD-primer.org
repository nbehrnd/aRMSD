...
#+TITLE:   aRMSD-primer (aRMSD 1.0.0)
#+DATE:    [2026-07-14 Mon]
...
* COMMENT Setup of aRMSD in Linux Debian / Ubuntu
  ...
  =aRMSD= running in Windows, you may skip =cython= which will provide
...
* COMMENT Example comparing two models successfully
  ...
  + From the top directory of =aRMSD=, launch the program from the
    ...
    name pointing to an existing model, but are not interested to
  ...
  + It is normal that performing the same computation a twice, with
    ...
    /slightly different/ from each other, e.g. between Xubuntu (point
...
* COMMENT Example comparing two models unsuccessfully
...
on a copy of the GitHub project resolves the requirements from
pypi.org.  Reserve about 0.8GB of permanent memory for this support.
When successful, the installation provides =armsd= as a new command to
the CLI.

** workflow

Except for widgets to provide an interactive structure display or user
adjustable diagrams about the test's statistics provided by
matplotlib, aRMSD lives in the command line.  The following tree
diagram recapituates key points of the workflow on .xyz data.
...
#+end_src
  ...
  folder =examples=, the optional log file written by =aRSMD= is
  provided below:

  #+INCLUDE: "./docSources/2026-07-13-aRMSD_logfile.out" src shell

  The complete log of a test run with =M1.xyz= and =M2.xyz= as test
  data, the report back to the BASH shell is provided below.  Host
  system was an instance of Linux Debian 13/trixie, supported by
  Python 3.13.5

  #+INCLUDE: "./docSources/2026-07-13-cli_log.log" src shell
* COMMENT Supplementary data
