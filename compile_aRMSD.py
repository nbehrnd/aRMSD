...
def pyinstaller_data(name, platform, obf_files):
    """ Sets up a PyInstaller dictionary with all parameters that will be
...
def analyze_arguments(arguments):
    """ Checks given arguments and passes correct ones
        to the compilation script """
...
def get_current_version(armsd_dir):
    """ Returns the name of the executable (this part is copied
        from the spec file) """
...
def copy_obfiles(build_dir, site_packages_path):
    ...
    # List of .obf files (file format support), add or
    # remove obf files accordingly
...
def write_ob_hook(site_packages_path, overwrite):
    ...
    if not os.path.isfile(hook_path+'\\hook-openbabel.py') or overwrite:
        ...
        # and fall back to the internal data
