...
from . import acore as ac
from . import aplot as ap
from . import alog as al
...
__aRMSD_version__ = '1.0.1'
__aRMSD_release__ = 2026

__log_version__ = '2026-07-14'
__core_version__ = '2026-07-22'
...
documentation under """+str(doc)+""". After installing the package
(pip install .), run it from a shell with

armsd

or, without installing the console script, from Python with
...
def main():
    ...
    Wraps run() and discards its return value. run() returns the 'align'
    object for programmatic/library use (e.g. `align = armsd.run()`), but
    a console-script wrapper generated by pip does `sys.exit(main())`, and
    a non-None, non-int return value there gets printed to stderr and
    causes a non-zero exit status. main() avoids that by not returning
    anything meaningful. """
